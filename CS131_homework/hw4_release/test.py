...
#             if(j == 0):
#                 cost[i][j] = energy[i][j] + \
#                     np.min([np.abs(image[i][j + 1]) + np.abs(image[i - 1][j]), cost[i - 1][j] + np.abs(image[i][j + 1]),
#                             cost[i - 1][j + 1] + np.abs(image[i][j + 1]) + np.abs(image[i - 1][j] - image[i][j + 1])])
#                 paths[i][j] = np.argmin(
#                     [cost[i - 1][j], cost[i - 1][j + 1]])
#             elif(j == W - 1):
#                 cost[i][j] = energy[i][j] + \
#                     np.min([cost[i - 1][j - 1] + np.abs(image[i][j - 1]) + np.abs(image[i - 1][j] - image[i][j - 1]),
#                             cost[i - 1][j] + np.abs(image[i][j - 1]), np.abs(image[i][j - 1]) + np.abs(image[i - 1][j])])
#                 paths[i][j] = np.argmin(
#                     [cost[i - 1][j - 1], cost[i - 1][j]]) - 1
#             else:
#                 l = np.abs(image[i][j + 1] - image[i][j - 1]) + \
#                     np.abs(image[i - 1][j] - image[i][j - 1])
#                 v = np.abs(image[i][j + 1] - image[i][j - 1])
#                 r = np.abs(image[i][j + 1] - image[i][j - 1]) + \
#                     np.abs(image[i - 1][j] - image[i][j + 1])
#                 cost[i][j] = energy[i][j] + \
#                     np.min([cost[i - 1][j - 1] + np.abs(l), cost[i - 1]
#                             [j] + np.abs(v), cost[i - 1][j + 1] + np.abs(r)])
#                 paths[i][j] = np.argmin([cost[i - 1][j - 1], cost[i - 1]
#                                          [j], cost[i - 1][j + 1]]) - 1
#     # END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1, H):
        for j in range(0, W):
            if(j == 0):
                cost[i][j] = energy[i][j] + \
                    np.min([cost[i - 1][j] + np.abs(image[i][j + 1]),
                            cost[i - 1][j + 1] + np.abs(image[i - 1][j] - image[i][j + 1])])
                paths[i][j] = np.argmin(
                    [cost[i - 1][j], cost[i - 1][j + 1]])
            elif(j == W - 1):
                cost[i][j] = energy[i][j] + \
                    np.min([cost[i - 1][j - 1] + np.abs(image[i - 1][j] - image[i][j - 1]),
                            cost[i - 1][j] + np.abs(image[i][j - 1])])
                paths[i][j] = np.argmin(
                    [cost[i - 1][j - 1], cost[i - 1][j]]) - 1
            else:
                l = np.abs(image[i][j + 1] - image[i][j - 1]) + \
...
# # Vertical Cost Map
# # don't need the first argument for compute_cost
# vcost, vpaths = compute_forward_cost(img_test, energy)
